...
type ActivityResponse struct {
	...
	Modified map[string]int `json:"modified"`
}

func (idx *Indexer) GetActivityDates() (ActivityResponse, error) {
	resp := ActivityResponse{
		Created:  make(map[string]int),
		Modified: make(map[string]int),
	}

	// 1. Get Created Dates (MIN line_num for each day something was created)
	rows, err := idx.db.Query("SELECT date(created_at, 'unixepoch', 'localtime'), MIN(line_num) FROM timeline WHERE created_at != 0 AND TRIM(text) != '' GROUP BY 1")
	if err != nil {
		return resp, err
	}
	for rows.Next() {
	...
	// 2. Get Modified Dates (MIN line_num for each day something was modified)
	rows, err = idx.db.Query("SELECT date(timestamp, 'unixepoch', 'localtime'), MIN(line_num) FROM timeline WHERE timestamp != 0 AND TRIM(text) != '' GROUP BY 1")
	if err != nil {
		return resp, err
	}
	for rows.Next() {
